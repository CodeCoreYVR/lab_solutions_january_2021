...
-- 1. Select the product whose stock has the most value (use sale price)
  SELECT name, (sale_price * remaining_quantity) AS total_value
  FROM products
...
-- 9. Update all the products whose name contains `paper` or `steel` to have a remaining quantity of a random number between 5 and 25
  UPDATE products
  SET remaining_quantity = (RANDOM() * 21 + 5)
  WHERE name ILIKE '%paper%' OR name ILIKE '%steel%';

  SELECT * FROM products WHERE name ILIKE '%paper%' OR name ILIKE '%steel%';

-- 10. Select the second set of 10 cheapest products with remaining quantity
